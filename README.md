...
- Atlas 自重启由独立 systemd 定时单元执行，避免 agent 停止时杀死重启命令。90 秒回滚保护仍然保留，新进程稳定运行至少 30 秒且相关服务健康后解除保护。
- 控制器收到更新结果后保持任务运行中，临时要求每 10 秒回报版本；至少等待 3 分钟跨过旧版 agent 的回滚窗口，并收到连续至少 30 秒匹配目标版本的报告才标记成功。5 分钟内无法确认则失败，不自动重新排队；心跳和缓存版本不能代替新报告。
- 从存在自重启竞态的旧版 agent 首次升级时，应从 agent 服务之外执行有备份的二进制替换与重启。仅发布新版本无法修复仍由旧进程执行的升级流程。先更新控制器，再升级节点；历史已完成任务不会被追溯改写。
