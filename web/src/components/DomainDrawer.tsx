...
export function DomainDrawer({ open, nodes, certificates, dnsAccounts, acmeAccounts, busy, onClose, onSubmit }: Props) {
  ...
  useEffect(() => {
    ...
    setError('')
  // Resource arrays refresh while the form is open. Only initialize at open.
  }, [open])
  ...
  useEffect(() => {
    ...
    if (!eligibleCertificates.some((certificate) => certificate.id === certificateID)) {
  ...
  return (
    <div className="drawer-layer drawer-open" onMouseDown={(event) => event.currentTarget === event.target && !busy && onClose()}>
      <form className="domain-drawer" onSubmit={submit} aria-label={t('domain.add')}>
        <header className="drawer-header"><div><span className="dialog-kicker">ROUTE DEPLOYMENT</span><h2>{t('domain.add')}</h2><p>{t('domain.drawerSubtitle')}</p></div><IconButton name="close" label={t('common.close')} type="button" onClick={onClose} disabled={busy} /></header>
